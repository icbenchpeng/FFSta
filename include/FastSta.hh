...
//   using RTaggedDataForm = std::unordered_map<CTaggedVertex, RTaggedData*, CTaggedVertexHash>;
 ...
 public: // compile
  ...
  void compileDelayCalc(CTaggedVertex from, CTaggedVertex to, TimingArc* arc, Edge* edge, bool to_propagates_clk, const PathAnalysisPt* path_ap) {
  ...
  struct RTaggedDataForm {
    ...
    std::vector<RTaggedData*> get(Vertex* vertex) const {
      ...
      for (auto [tag, arrival_index] : *(tags->arrivalMap())) {
        res.push_back(get(CTaggedVertex{vertex, tag}));
      }
      return res;
    }
